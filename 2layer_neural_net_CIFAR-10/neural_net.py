...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    tmp = np.exp(scores)
    act2 = tmp/(np.sum(tmp, axis=1, keepdims=True))
    # loss=data_loss+regularization_loss
    data_loss = (-1/N)*(np.sum(np.log(act2[range(N), y])))
    reg_loss = reg * np.sum(W2*W2)+ np.sum(W1*W1)*reg
    ...
    dW2 = np.dot(act1.T, ds)
    db2=np.sum(ds,axis=0).reshape(W2.shape[1])
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in range(num_iters):
      ...
      #########################################################################
	  # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        tmp = np.random.choice(np.arange(num_train), batch_size)
  ...
  def predict(self, X):
    ...
    act1 = np.maximum(zeros, z1)
    scores = act1.dot(self.params['W2']) + self.params['b2']
