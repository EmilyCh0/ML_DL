import torch.nn as nn
import torch

###########################################################################
# Question 1 : Implement the UNet model code.
# Understand architecture of the UNet in practice lecture 15 -> slides 5-6 (30 points)

def conv(in_channels, out_channels):
    return nn.Sequential(
        nn.Conv2d(in_channels, out_channels, 3, padding=1),  # 3은 kernel size
        nn.BatchNorm2d(out_channels),
        nn.ReLU(inplace=True),
        nn.Conv2d(out_channels, out_channels, 3, padding=1),
        nn.BatchNorm2d(out_channels),
        nn.ReLU(inplace=True)
    )
######
# def conv_trans_block(in_dim,out_dim):
#     model = nn.Sequential(
#         nn.ConvTranspose2d(in_dim,out_dim, kernel_size=3, stride=2, padding=1,output_padding=1),
#         nn.BatchNorm2d(out_dim),
#         nn.ReLU(inplace=True),
#     )
#     return model
######

class Unet(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(Unet, self).__init__()

        ########## fill in the blanks (Hint : check out the channel size in practice lecture 15 ppt slides 5-6)
        self.convDown1 = conv(in_channels, 64)
        self.convDown2 = conv(64, 128)
        self.convDown3 = conv(128, 256)
        self.convDown4 = conv(256, 512)
        self.convDown5 = conv(512, 1024) #bridge
        self.maxpool = nn.MaxPool2d(2, stride=2)
        self.upsample = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
        # self.convUp4 = conv(1024, 512)
        # self.convUp3 = conv(512, 256)
        # self.convUp2 = conv(256, 128)
        # self.convUp1 = conv(128, 64)

        self.convUp4 = conv(1536, 512)
        self.convUp3 = conv(768, 256)
        self.convUp2 = conv(384, 128)
        self.convUp1 = conv(192, 64)
        self.convUp_fin = nn.Conv2d(64, out_channels, 1)

        # self.trans1 = conv_trans_block(1024,512)
        # self.convUp1 = conv(1024, 512)
        # self.trans2 = conv_trans_block(512, 256)
        # self.convUp2 = conv(512, 256)
        # self.trans3 = conv_trans_block(256, 128)
        # self.convUp3 = conv(256, 128)
        # self.trans4 = conv_trans_block(128, 64)
        # self.convUp4 = conv(128, 64)
        # self.convUp_fin = nn.Conv2d(64, out_channels, 3,1,1)


    def forward(self, x):
        conv1 = self.convDown1(x)
        x = self.maxpool(conv1)
        conv2 = self.convDown2(x)
        x = self.maxpool(conv2)
        conv3 = self.convDown3(x)
        x = self.maxpool(conv3)
        conv4 = self.convDown4(x)
        x = self.maxpool(conv4)
        #bridge
        conv5 = self.convDown5(x)
        x = self.upsample(conv5)
        #######fill in here
        x = torch.cat([x,conv4],dim=1) ##
        ####### hint : concatenation (Practice Lecture slides 6p)
        x = self.convUp4(x)
        x = self.upsample(x)
        #######fill in here
        x = torch.cat([x, conv3], dim=1)  ##
        # ####### hint : concatenation (Practice Lecture slides 6p)
        x = self.convUp3(x)
        x = self.upsample(x)
        #######fill in here
        x = torch.cat([x, conv2], dim=1)  ##
        # ####### hint : concatenation (Practice Lecture slides 6p)
        x = self.convUp2(x)
        x = self.upsample(x)
        #######fill in here
        x = torch.cat([x, conv1], dim=1)  ##
        # ####### hint : concatenation (Practice Lecture slides 6p)
        x = self.convUp1(x)
        out = self.convUp_fin(x)

        # #bridge
        # conv5 = self.convDown5(x)
        #
        # x=self.trans1(conv5)
        # x = torch.cat([x, conv4], dim=1)
        # up1=self.convUp1(x)
        # x = self.trans1(up1)
        # x = torch.cat([x, conv3], dim=1)
        # up2 = self.convUp1(x)
        # x = self.trans1(up2)
        # x = torch.cat([x, conv2], dim=1)
        # up3 = self.convUp1(x)
        # x = self.trans1(up3)
        # x = torch.cat([x, conv1], dim=1)
        # up4 = self.convUp1(x)
        #
        # out = self.convUp_fin(up4)

        return out
